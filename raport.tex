...
normalizuje współczynniki wzrostu i~zaokrągla każdy z~nich w~dół do
najbliższej potęgi~$1/2$. Bezkolizyjne okresowe terminy cięć są następnie
reprezentowane przez drzewo prefiksowe. Niewykorzystane terminy mogą zostać
przeznaczone na dodatkowe cięcie według \RM{}, co nie pogarsza gwarancji.
Jest to prostsza realizacja harmonogramu dyadycznego odpowiednia dla małych
instancji wizualizowanych w~aplikacji; nie implementuje zoptymalizowanego
drzewa wirtualnych bambusów używanego w~\cite{bilo2022} do uzyskania
czasu zapytania $O(\log n)$.
